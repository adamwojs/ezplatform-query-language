...
final class QueryVisitorTest extends TestCase
{
    ...
    public function dataProviderForSelectContent(): iterable
    {
        ...
        foreach ($this->dataProviderForCriterion() as $name => [$expression, $criterion]) {
        ...
        foreach ($this->dataProviderForSortClauses() as $name => [$expression, $sortClauses]) {
            yield 'sort ' . $name => [
    ...
    public function dataProviderForCriterion(): iterable
    {
        yield 'match all' => [
            'MATCH ALL',
            new MatchAll(),
        ...
        foreach ($this->dataProviderForOperator() as $op => [$tail, $value]) {
            yield "field is $op" => [
                "FIELD foo $tail",
        ...
        foreach ($this->dataProviderForLocationPriorityCriterion() as $name => [$expression, $criterion]) {
            yield $name => [
                $expression,
                $criterion,
            ];
        ...
        foreach ([UserMetadata::MODIFIER, UserMetadata::GROUP, UserMetadata::OWNER] as $target) {
            foreach ($this->dataProviderForUserMetadata($target) as $name => [$expression, $criterion]) {
                yield $name => [
                    $expression,
                    $criterion,
                ];
    ...
    public function dataProviderForLocationPriorityCriterion(): iterable
    {
    ...
    public function dataProviderForUserMetadata(string $target): iterable
    {
        yield "$target =" => [
    ...
    public function dataProviderForOperator(): iterable
    {
    ...
    private function dataProviderForSortClauses(): iterable
    {
